...
export default function App() {
  ...
  return (
    ...
    <div className="min-h-[100dvh] overflow-x-hidden selection:bg-accent selection:text-bg bg-bg text-ink font-sans transition-colors duration-300 flex flex-col md:flex-row pt-20 md:pt-0">
      
      {/* Mobile Navbar */}
      <nav className="fixed top-0 w-full z-50 bg-bg/80 backdrop-blur-md border-b border-ink/10 md:hidden">
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed inset-0 z-[100] bg-bg flex flex-col p-6 md:hidden"
          >
      ...
      <aside className="w-full md:w-72 bg-surface border-r border-ink/10 md:h-screen md:fixed flex flex-col md:pt-10 p-6 z-10 hidden md:flex">
        ...
        >
          ...
          <p className="text-xs text-ink/50 mt-1 font-mono">ID: MZ-OPS-2026</p>
        </button>
        ...
        <nav className="flex flex-col gap-2">
          <button 
            ...
            className={`flex items-center px-4 py-3 rounded-xl text-sm font-medium transition-colors ${activeTab === 'overview' ? 'bg-accent text-bg shadow-sm' : 'hover:bg-ink/5 text-ink/70'}`}
          >
            <LayoutDashboard size={18} className="mr-3" /> Overview
          </button>
          <button 
            onClick={() => setActiveTab('experience')} 
            className={`flex items-center px-4 py-3 rounded-xl text-sm font-medium transition-colors ${activeTab === 'experience' ? 'bg-accent text-bg shadow-sm' : 'hover:bg-ink/5 text-ink/70'}`}
          >
            <Briefcase size={18} className="mr-3" /> {currentT.nav.exp}
          </button>
          <button 
            onClick={() => setActiveTab('skills')} 
            className={`flex items-center px-4 py-3 rounded-xl text-sm font-medium transition-colors ${activeTab === 'skills' ? 'bg-accent text-bg shadow-sm' : 'hover:bg-ink/5 text-ink/70'}`}
          >
            <Code size={18} className="mr-3" /> {currentT.nav.skills}
          </button>
          <button 
            onClick={() => setActiveTab('contact')} 
            className={`flex items-center px-4 py-3 rounded-xl text-sm font-medium transition-colors ${activeTab === 'contact' ? 'bg-accent text-bg shadow-sm' : 'hover:bg-ink/5 text-ink/70'}`}
          >
        ...
        <div className="mt-auto pt-6 border-t border-ink/10 flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between px-2">
            <span className="text-sm font-medium text-ink/70">Langue</span>
            <button 
      ...
      <main className="flex-1 md:ml-72 p-6 md:p-10 lg:p-12 max-w-5xl w-full flex flex-col">
        <AnimatePresence mode="wait">
          
          {/* TAB 1: OVERVIEW */}
          {activeTab === 'overview' && (
            <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4 md:space-y-6 my-auto w-full py-2">
              <header className="mb-4 md:mb-6">
                <h2 className="text-3xl md:text-4xl font-display font-bold mb-2">Dashboard</h2>
                <p className="text-ink/60 text-lg">{currentT.role}</p>
              </header>
                ...
                {currentT.experience.items.map((exp: Experience, i: number) => (
                  <div key={i} className="bg-surface border border-accent/20 md:border-ink/10 p-6 rounded-2xl shadow-sm flex flex-col md:flex-row gap-4 md:gap-6 md:hover:border-accent/30 transition-colors group">
                    ...
                    <div>
                      ...
                      <p className="text-ink/70 leading-relaxed text-sm md:text-base">{exp.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
